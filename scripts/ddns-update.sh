...
# Updates DNS A record with Pi's public IP
# ============================================
...
if [ -z "$CF_API_TOKEN" ] || [ -z "$CF_ZONE_ID" ]; then
  echo "[ddns] ERROR: CF_API_TOKEN and CF_ZONE_ID must be set in $INSTALL_DIR/.env"
  exit 1
fi
...
if [ -z "$RECORD_ID" ]; then
  ...
  echo "[ddns] Updating A record: ${CF_RECORD_NAME} ${OLD_IP} → ${CURRENT_IP}"
  curl -s -X PUT \
